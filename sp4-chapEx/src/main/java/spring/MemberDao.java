...
public class MemberDao {
	...
	public Member selectByEmail(String email) {
		...
		List<Member> results = jdbcTemplate.query("select * from spring4fs.MEMBER where EMAIL=?", memRowMapper,
				email);
	...
	public List<Member> selectByRegdate(Date from, Date to) {
		List<Member> results = jdbcTemplate.query(
				"select * from spring4fs.MEMBER where REGDATE between ? and ? " + "order by REGDATE desc",
	...
	public void update(Member member) {
		jdbcTemplate.update("update spring4fs.MEMBER set NAME=?, PASSWORD=? where EMAIL=?", member.getName(),
	...
	public int count() {
		Integer count = jdbcTemplate.queryForObject("select count(*) from spring4fs.MEMBER", Integer.class);
		return count;
	}
